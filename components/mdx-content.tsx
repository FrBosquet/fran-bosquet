...
const code = (props: HTMLAttributes<HTMLElement>) => {
  ...
  if (!isBlock) return <code {...props} className="bg-gray-900 text-teal-200 px-1 rounded-md inline-block leading-8">{children}</code>
...
const mdxComponents = {
  ...
  h4: (props: HTMLAttributes<HTMLHeadingElement>) => <h3 className='text-xl text-orange-100 mb-1 '>{props.children}</h3>,
  p: (props: HTMLAttributes<HTMLParagraphElement>) => <p className='font-medium text-sm leading-7 md:text-lg pb-3 md:leading-10 tracking-wide md:tracking-normal'>{props.children}</p>,
  img: (props: any) => <Image width={700} height={400} className="w-auto mb-1 m-auto rounded-lg shadow-lg" title={props.alt} placeholder='blur' blurDataURL={`/images/placeholder/${props.src}.webp`} alt={props.alt} src={`/images/${props.src}.webp`} />,
