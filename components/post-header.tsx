...
export const PostHeader = ({ title, date, keywords, image, description, lang, slug }: Props) => {
	return <>
		<header className="border-b-2 border-gray-500 pb-2 mb-4">
			<h1 style={{
				viewTransitionName: slug
			}} className="text-4xl md:text-5xl bg-gradient-to-r from-teal-100 to-orange-200 text-transparent py-2 bg-clip-text font-mono">{title}</h1>

			{description ? <p
				style={{
					viewTransitionName: `desc-${slug}`
				}}
				className="text-sm uppercase text-teal-200 py-4">{description}</p> : null}

			{image ? <><div className="w-full h-60 relative rounded-lg overflow-hidden my-2 shadow-lg">
				<Image title="splash" alt="splash" src={`/images/${image.src}`} fill style={{ objectFit: 'cover', viewTransitionName: `image-${slug}` }}></Image>
